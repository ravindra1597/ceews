...
const ai = new GoogleGenAI({ apiKey: 'AIzaSyCNKW28r38UlFnClDAtu0-2MSjFq2_XB3o' });

const firebaseConfig = {
  apiKey: "AIzaSyCNKW28r38UlFnClDAtu0-2MSjFq2_XB3o",
...
export default function App() {
  ...
  const [vitals, setVitals] = useState({ heartRate: 75, systolicBP: 120, spo2: 98, troponin: 0.01 });
  const [history, setHistory] = useState({
    heartRate: Array(20).fill(75),
    systolicBP: Array(20).fill(120),
    spo2: Array(20).fill(98),
  });

  const [riskScore, setRiskScore] = useState(12);
  const [riskLevel, setRiskLevel] = useState('Low');
  const [riskBreakdown, setRiskBreakdown] = useState({ hr: 33, bp: 33, trop: 34 });
  ...
  const callGeminiAgent = async (currentVitals) => {
...
5. [Fifth clinical action]`;
  ...
  const analyzePatient = (currentVitals) => {
    const hrTrend = currentVitals.heartRate - history.heartRate[history.heartRate.length - 1];
    const bpTrend = currentVitals.systolicBP - history.systolicBP[history.systolicBP.length - 1];

    let hrWeight = Math.abs(currentVitals.heartRate - 75) * 1.2;
    let bpWeight = Math.abs(120 - currentVitals.systolicBP) * 1.5;
    let tropWeight = (currentVitals.troponin / 0.1) * 100;

    if (hrTrend > 5) hrWeight *= 1.5;
    if (bpTrend < -5) bpWeight *= 1.8;

    const total = hrWeight + bpWeight + tropWeight;
  ...
  useEffect(() => {
    ...
    if (isLive) {
      interval = setInterval(() => {
        setVitals(prev => {
          let newHR = prev.heartRate + (Math.random() * 4 - 2);
          let newBP = prev.systolicBP + (Math.random() * 6 - 3);
          let newSpo2 = Math.min(100, prev.spo2 + (Math.random() * 2 - 1));
          let newTrop = prev.troponin;

          if (stressTestActive) { newHR += 5; newBP -= 4; newSpo2 -= 1; newTrop += 0.005; }
          ...
          setHistory(h => ({
            heartRate: [...h.heartRate.slice(1), nextVitals.heartRate],
            systolicBP: [...h.systolicBP.slice(1), nextVitals.systolicBP],
            spo2: [...h.spo2.slice(1), nextVitals.spo2],
          }));

          analyzePatient(nextVitals);
          return nextVitals;
        });
      }, 1500);
    }
    return () => clearInterval(interval);
  }, [isLive, stressTestActive]);
  ...
  const triggerStressTest = () => {
    ...
    setTimeout(() => {
      callGeminiAgent({ heartRate: 165, systolicBP: 72, spo2: 84, troponin: 0.18 });
    }, 3000);
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-5">
          ...
          <div className="lg:col-span-4 space-y-4">
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200 flex items-center space-x-4">
              ...
              <div>
                <p className="text-xs text-gray-400 uppercase tracking-wider font-semibold">ICU Bed 4</p>
                <p className="font-bold text-gray-900">Patient Telemetry</p>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-3xl font-mono font-bold ${vitals.heartRate > 110 || vitals.heartRate < 50 ? 'text-[#dc2626]' : 'text-[#16a34a]'}`}>
                  {vitals.heartRate}<span className="text-sm font-sans text-gray-400 ml-1 font-normal">bpm</span>
                </span>
              </div>
              <Sparkline
                data={history.heartRate}
                color={vitals.heartRate > 110 || vitals.heartRate < 50 ? '#dc2626' : '#16a34a'}
                min={40} max={160}
              />
            </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-3xl font-mono font-bold ${vitals.systolicBP > 160 || vitals.systolicBP < 90 ? 'text-[#dc2626]' : 'text-gray-800'}`}>
                  {vitals.systolicBP}<span className="text-sm font-sans text-gray-400 ml-1 font-normal">mmHg</span>
                </span>
              </div>
              <Sparkline
                data={history.systolicBP}
                color={vitals.systolicBP > 160 || vitals.systolicBP < 90 ? '#dc2626' : '#2563eb'}
                min={60} max={200}
              />
            </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200">
              <div className="flex items-center justify-between">
                ...
                <span className={`text-3xl font-mono font-bold ${vitals.spo2 < 94 ? 'text-[#dc2626]' : 'text-[#16a34a]'}`}>
                  {vitals.spo2}<span className="text-sm font-sans text-gray-400 ml-1 font-normal">%</span>
                </span>
              </div>
              <Sparkline
                data={history.spo2}
                color={vitals.spo2 < 94 ? '#dc2626' : '#16a34a'}
                min={70} max={100}
              />
            </div>
            ...
            <div className="bg-white rounded-xl p-4 border border-gray-200">
              <div className="flex items-center justify-between mb-3">
                ...
                <span className={`text-3xl font-mono font-bold ${vitals.troponin > 0.04 ? 'text-[#dc2626]' : 'text-[#d97706]'}`}>
                  {vitals.troponin.toFixed(3)}<span className="text-sm font-sans text-gray-400 ml-1 font-normal">ng/mL</span>
                </span>
              </div>
              <div className="w-full bg-gray-100 rounded-full h-2">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${vitals.troponin > 0.04 ? 'bg-[#dc2626]' : 'bg-[#d97706]'}`}
                  style={{ width: `${Math.min((vitals.troponin / 0.1) * 100, 100)}%` }}
                />
              </div>
          ...
          <div className="lg:col-span-8 space-y-5">
            ...
            {/* ── RISK STATUS CARD ── */}
            <div
              className={`rounded-2xl border-2 p-6 transition-all duration-500 ${riskConfig.cardBg} ${riskConfig.cardBorder} ${riskConfig.isPulsing ? 'critical-card-glow' : ''}`}
            >
              <div className="flex flex-col md:flex-row items-center justify-between gap-6">
                <div className="flex-1 min-w-0">
                  ...
                  {/* THE BADGE — minimum 48px, dominant element */}
                  <div
